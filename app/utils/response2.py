...
def process_token(uid, password):
    ...
    game_data.os_info = f"{system_name} {release} ({version})"
    game_data.device_model = platform.node()
    cpu_name = platform.processor() or "Generic CPU"
    game_data.cpu_info = f"{cpu_name} | {psutil.cpu_freq().max:.0f} | {psutil.cpu_count(logical=False)}"
    game_data.total_ram = round(psutil.virtual_memory().total / (1024 * 1024))  # in MB
    disk = psutil.disk_usage('/')
    game_data.total_storage = round(disk.total / (1024 * 1024))  # in MB
    game_data.gpu_name = "Adreno/Generic GPU"
    ...
    try:
        ...
        local_ip = "127.0.0.1"
    game_data.ip_address = local_ip

    game_data.game_name = "free fire"
    game_data.game_version = 1
    game_data.version_code = "2.115.14"
